...
export default function Navbar() {
  ...
  const linkStyles = useMemo(() => {
    const active = {
      ...
      background: "var(--accent-light)", whiteSpace: "nowrap" as const,
    ...
    <nav style={{ background: "var(--primary-dark)", color: "white", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 3px 10px rgba(0,0,0,0.3)" }}>
      <div style={{ maxWidth: 1900, margin: "0 auto", padding: "0 0.5rem", height: 60, display: "flex", alignItems: "center", gap: "0" }}>
        ...
        <Link href="/" aria-label="ΚΑΔ 2025 αρχική" style={{ textDecoration: "none", flexShrink: 0, marginRight: "0.75rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
            <span style={{ background: "rgba(255,255,255,0.15)", color: "var(--accent-light)", fontWeight: 900, fontSize: "15px", padding: "5px 7px", borderRadius: "8px", fontFamily: "Arial,sans-serif", lineHeight: 1 }}>08</span>
            <span style={{ color: "var(--accent-light)", fontSize: "15px", fontWeight: 700, lineHeight: 1 }}>→</span>
            <span style={{ background: "rgba(255,255,255,0.15)", color: "var(--accent-light)", fontWeight: 900, fontSize: "15px", padding: "5px 7px", borderRadius: "8px", fontFamily: "Arial,sans-serif", lineHeight: 1 }}>25</span>
          </div>
          ...
          <span style={{ display: "flex", alignItems: "baseline", gap: "0", lineHeight: 1 }}>
            ...
            <sup style={{ fontSize: "0.65rem", fontWeight: 800, color: "var(--accent-light)", fontFamily: "Inter,Arial,sans-serif", marginLeft: "1px", verticalAlign: "super", lineHeight: 0 }}>.gr</sup>
          </span>
        </Link>
          ...
          <div ref={toolsRef} style={{ position: "relative", flexShrink: 0 }}>
            <button
              ...
              style={{
                ...
                background: toolsOpen ? "var(--accent-light)" : "rgba(255,255,255,0.1)",
                whiteSpace: "nowrap",
                border: toolsOpen ? "2px solid var(--accent-light)" : "1px solid rgba(255,255,255,0.25)",
                cursor: "pointer",
                display: "flex",
                alignItems: "center",
                gap: "0.3rem",
                ...
                {toolsLinks.map((t, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {(t as any).badge && <span style={{ fontSize: "0.65rem", padding: "0.1rem 0.3rem", borderRadius: 4, background: "var(--danger)", color: "white", fontWeight: 700 }}>{(t as any).badge}</span>}
                    </Link>
                  );
                })}
              </div>
                ...
                {searchResults.length > 0 && (
                  <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", maxHeight: 280, overflowY: "auto" }}>
                    {searchResults.map(r => (
                      ...
                      >
                        <span style={{ color: "var(--accent-light)", marginRight: 6 }}>→</span>{r.label}
                      </Link>
                    ))}
                  </div>
                )}
      ...
      {menuOpen && (
        <div style={{ background: "#0f2347", borderTop: "1px solid rgba(255,255,255,0.1)", maxHeight: "80vh", overflowY: "auto" }}>
          {allLinks.map(l => (
            <Link key={l.href} href={l.href} style={{ display: "block", color: pathname === l.href ? "var(--accent-light)" : "rgba(255,255,255,0.9)", textDecoration: "none", padding: "0.7rem 1.2rem", fontSize: "0.9rem", fontWeight: pathname === l.href ? 700 : 400, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              {l.label}
            </Link>
          ))}
          ...
          {toolsLinks.map((t, idx) => {
            ...
            if (t.isHeader) return (
              <Link key={t.href} href={t.href!} onClick={() => setMenuOpen(false)}
                style={{ display: "block", color: "var(--accent-light)", textDecoration: "none", padding: "0.5rem 1.2rem 0.3rem", fontSize: "0.72rem", fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                {t.label}
              </Link>
            );
            return (
              <Link key={t.href} href={t.href!} onClick={() => setMenuOpen(false)}
                ...
                <span style={{ flex: 1 }}>{t.label}</span>
                {t.badge && <span style={{ fontSize: "0.65rem", padding: "0.1rem 0.3rem", borderRadius: 4, background: "var(--danger)", color: "white", fontWeight: 700 }}>{t.badge}</span>}
              </Link>
            );
          })}
      ...
      `}</style>
        ...
        <div className="nav-ad-row">
          ...
          <Link href="/diafimisi" className="nav-ad-link" title="Διαφημιστείτε στο kad2025.gr">
            ...
            Διαφημιστική προβολή στο kad2025.gr
