...
function generateUniqueParagraphs(
  ...
  // v141: 906 κωδικοί έχουν ίδιο αριθμό αλλά ΑΛΛΑΞΕ η επίσημη περιγραφή τους.
  // Δεν είναι «αμετάβλητοι» — η έκταση της δραστηριότητας μπορεί να διαφέρει.
  const descChanged = r.desc2008.trim() !== r.desc2025.trim();
  const statusLabel = changed ? "Άλλαξε ο κωδικός" : descChanged ? "Ίδιος κωδικός, νέα περιγραφή" : "Αμετάβλητος";
  const statusColor = changed ? "var(--accent)" : descChanged ? "#d97706" : "var(--success)";
  ...
  if (changed && splitCount > 0) {
    ...
    if (splitCount > 0) {
      ...
      paragraphs.push(
        r.desc2008.trim() !== r.desc2025.trim()
          ? `Ο ΚΑΔ ${r.kad2008} διατηρεί τον ίδιο αριθμό στη νέα ονοματολογία ΚΑΔ 2025, αλλά η επίσημη περιγραφή του άλλαξε: από «${r.desc2008}» σε «${r.desc2025}», σύμφωνα με την απόφαση ΑΑΔΕ Α.1003/2026 που ισχύει από την 1η Μαρτίου 2026. Ο αριθμός δεν χρειάζεται μεταβολή στο Μητρώο, ωστόσο αξίζει να ελέγξετε αν η νέα, ευρύτερη ή στενότερη περιγραφή καλύπτει ακριβώς τη δραστηριότητά σας — και να δείτε τη συνολική εικόνα των ΚΑΔ σας στο myAADE.`
          : `Ο ΚΑΔ ${r.kad2008} (${r.desc2008}) παρέμεινε αμετάβλητος στη νέα ονοματολογία ΚΑΔ 2025, σύμφωνα με την απόφαση ΑΑΔΕ Α.1003/2026 που ισχύει από την 1η Μαρτίου 2026. Τόσο ο αριθμός όσο και η επίσημη περιγραφή παραμένουν ίδιοι, οπότε δεν προκύπτει ενέργεια για τον συγκεκριμένο κωδικό — ελέγξτε ωστόσο τη συνολική εικόνα Μητρώου και τους υπόλοιπους ΚΑΔ σας στο myAADE.`
      );
    }
  }
  ...
  if (changed) {
    paragraphs.push(
      `Τι πρέπει να κάνετε: συνδεθείτε στο myAADE (Μητρώο & Επικοινωνία → Βεβαιώσεις Μητρώου → Τρέχουσα Εικόνα Οντότητας) και επιβεβαιώστε ότι ο ΚΑΔ ${r.kad2025} αποδίδει σωστά τη δραστηριότητά σας. Αν χρειάζεται διόρθωση, υποβάλλεται μέσω της εφαρμογής «Μεταβολή Εργασιών» χωρίς πρόστιμο (εφόσον δεν έχει προηγηθεί δήλωση μεταβολής ΚΑΔ για την έδρα ή υποκατάστημα μετά την 1/3/2026, βάσει Α.1118/2026) έως τις 30 Οκτωβρίου 2026 (απόφαση Α.1118/2026). Ενημερώστε επίσης λογιστή, τιμολογιακά προγράμματα και τυχόν μητρώα (ΓΕΜΗ, επιμελητήρια) ώστε όλα τα συστήματα να αναφέρουν τον ίδιο κωδικό.`
    );
  }
...
export async function generateMetadata({
  ...
  if (isMulti) {
    ...
    const descChangedMeta = record.desc2008.trim() !== record.desc2025.trim();
    const titlePrefix = `ΚΑΔ ${record.kad2008} ${descChangedMeta ? "Νέα Περιγραφή" : "Αμετάβλητος"} | `;
  ...
  const faqItems = [
    {
      ...
      a: allMappings.length > 0
        ...
        : `Ο ΚΑΔ ${r.kad2008} (${r.desc2008}) αντιστοιχεί στον νέο ΚΑΔ ${r.kad2025} (${r.desc2025}). ${changed ? "Ο κωδικός άλλαξε με την εφαρμογή της NACE Rev.2.1." : descChanged ? "Ο αριθμός του κωδικού παραμένει ίδιος, αλλά η επίσημη περιγραφή του άλλαξε με τη NACE Rev.2.1." : "Ο κωδικός παρέμεινε αμετάβλητος."}`,
    },
    {
    ...
    {
      q: "Μέχρι πότε μπορώ να διορθώσω τον ΚΑΔ μου χωρίς πρόστιμο (εφόσον δεν έχει προηγηθεί δήλωση μεταβολής ΚΑΔ για την έδρα ή υποκατάστημα μετά την 1/3/2026, βάσει Α.1118/2026);",
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "2rem 1rem" }}>
      ...
      <div className="tldr">
        ...
        {changed
          ...
          : descChanged
          ? <>Ο ΚΑΔ {r.kad2008} διατηρεί τον <strong>ίδιο αριθμό</strong>, αλλά η επίσημη περιγραφή του <strong>άλλαξε</strong>. Ελέγξτε αν καλύπτει τη δραστηριότητά σας.</>
          : <>Ο ΚΑΔ {r.kad2008} <strong>παρέμεινε αμετάβλητος</strong>. Δεν προκύπτει ενέργεια για τον συγκεκριμένο κωδικό.</>
        }
      </div>
        ...
        <div className="card" style={{ padding: "1rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", fontSize: "0.875rem" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ fontWeight: 600, color: statusColor, textAlign: "right" }}>
                {statusLabel}
              </span>
            </div>
            {relatedOld.length > 0 && (
        ...
        {changed && (
          <div style={{ background: "var(--warn-bg)", border: "1px solid var(--warn-border)", borderRadius: "var(--radius)", padding: "1rem" }}>
            ...
            <p style={{ fontSize: "0.85rem", color: "var(--warn-text)", lineHeight: 1.6 }}>
              ...
              Διορθώστε έως <strong>30 Οκτωβρίου 2026</strong> χωρίς πρόστιμο (εφόσον δεν έχει προηγηθεί δήλωση μεταβολής ΚΑΔ για την έδρα ή υποκατάστημα μετά την 1/3/2026, βάσει Α.1118/2026) μέσω{" "}
