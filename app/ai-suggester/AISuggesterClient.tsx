  ...
  return (
    <div>
      <div className="card" style={{ marginBottom: "1.5rem" }}>
        ...
        <div style={{ background: "var(--warn-bg)", border: "1px solid var(--warn-border)", borderRadius: 8, padding: "0.6rem 0.75rem", marginBottom: "0.5rem", fontSize: "0.78rem", color: "var(--warn-text)", lineHeight: 1.5 }}>
          ⚠️ <strong>Σημαντικό:</strong> Μην εισάγετε προσωπικά δεδομένα, ΑΦΜ, επωνυμία ή εμπιστευτικές πληροφορίες. Η περιγραφή αποστέλλεται σε τρίτο πάροχο ΤΝ (Groq) για παραγωγή προτάσεων ΚΑΔ. <a href="/privacy" style={{ color: "var(--warn-strong)" }}>Πολιτική Απορρήτου</a>
        </div>
        <textarea
          value={text}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "0.75rem" }}>
          <span style={{ fontSize: "0.78rem", color: text.length >= MAX_CHARS ? "var(--danger)" : "var(--text-muted)" }}>
      ...
      {error && (
        <div style={{ background: "#fef2f2", border: "1px solid var(--danger-border)", borderRadius: "var(--radius)", padding: "1rem", color: "var(--danger)", marginBottom: "1rem" }}>
          ⚠️ {error}
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {results.map((s, i) => {
              ...
              return (
                <div key={i} className="card" style={{ borderLeft: `4px solid ${s.confidence >= 80 ? "var(--success)" : s.confidence >= 60 ? "var(--accent-light)" : "#94a3b8"}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem", flexWrap: "wrap" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.25rem", flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontSize: "0.75rem", background: s.confidence >= 80 ? "var(--success-bg)" : s.confidence >= 60 ? "#fef9c3" : "#f1f5f9", color: s.confidence >= 80 ? "var(--success-strong)" : s.confidence >= 60 ? "#854d0e" : "#475569", borderRadius: 20, padding: "0.15rem 0.6rem", fontWeight: 600 }}>
                    ...
                    <div style={{ flexShrink: 0 }}>
                      <Link href={`/kad-2025?q=${cleanCode}`} className="btn btn-ghost" style={{ fontSize: "0.8rem", padding: "0.4rem 0.9rem" }}>
                        Αναζήτηση ΚΑΔ 2025 →
                      </Link>
                    </div>
                  </div>
                  <div style={{ marginTop: "0.5rem", height: 4, borderRadius: 2, background: "var(--border)", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${s.confidence}%`, background: s.confidence >= 80 ? "var(--success)" : s.confidence >= 60 ? "var(--accent-light)" : "#94a3b8" }} />
                  </div>
                </div>
              );
            })}
