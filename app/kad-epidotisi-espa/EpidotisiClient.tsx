...
export default function EpidotisiClient() {
  ...
  return (
    <div className="card" style={{ marginBottom: "1.5rem" }}>
      ...
      {error && (
        <div style={{ color: "var(--danger)", background: "#fef2f2", padding: "0.75rem 1rem", borderRadius: 8, marginBottom: "1rem" }}>
          {error}
        </div>
      )}
      ...
      {result && (
        <div>
          ...
          {result.eligible.length === 0 && (
            <div style={{ background: "#fef2f2", border: "1.5px solid var(--danger-border)", borderRadius: 10, padding: "1rem 1.25rem", marginBottom: "1rem" }}>
              <div style={{ fontWeight: 700, color: "var(--danger)", marginBottom: "0.4rem" }}>
          ...
          {result.eligible.length > 0 && (
            <div style={{ background: "var(--success-bg)", border: "1.5px solid var(--success-border)", borderRadius: 10, padding: "1rem 1.25rem", marginBottom: "1rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {result.eligible.map(id => {
                  const p = PROGRAMS[id];
                  return (
                    <div key={id} style={{ display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.5rem 0.75rem", background: "var(--bg-card)", border: `2px solid ${p.color}`, borderRadius: 8 }}>
